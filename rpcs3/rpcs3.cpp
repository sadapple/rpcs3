...
const extern cfg::map_entry<std::function<std::shared_ptr<KeyboardHandlerBase>()>> g_cfg_kb_handler("io/Keyboard Handler",
{
	{ "Null", PURE_EXPR(std::make_shared<NullKeyboardHandler>()) },
	{ "Basic", PURE_EXPR(std::make_shared<BasicKeyboardHandler>()) },
});

const extern cfg::map_entry<std::function<std::shared_ptr<MouseHandlerBase>()>> g_cfg_mouse_handler("io/Mouse Handler",
{
	{ "Null", PURE_EXPR(std::make_shared<NullMouseHandler>()) },
	{ "Basic", PURE_EXPR(std::make_shared<BasicMouseHandler>()) },
});

const extern cfg::map_entry<std::function<std::shared_ptr<PadHandlerBase>()>> g_cfg_pad_handler("io/Pad Handler", "Keyboard",
{
...
const extern cfg::map_entry<std::function<std::shared_ptr<GSRender>()>> g_cfg_gs_render("rsx/Renderer", "OpenGL",
{
...
const extern cfg::map_entry<std::function<std::shared_ptr<AudioThread>()>> g_cfg_audio_render("audio/Audio Out", "OpenAL",
{
...
void Rpcs3App::OnArguments(const wxCmdLineParser& parser)
{
	...
	if (parser.FoundSwitch("t"))
	{
		...
		cfg::from_bool("misc/Always start after boot", true);
		cfg::from_bool("misc/Exit RPCS3 when process finishes", true);
	}
