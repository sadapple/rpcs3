...
void GameViewer::DClick(wxListEvent& event)
{
	...
	{
		LOG_ERROR(HLE, "Failed to boot %s", path);
	}
}
...
inline ColumnsArr::ColumnsArr()
{
	Init();
}

inline std::vector<Column*> ColumnsArr::GetSortedColumnsByPos()
{
...
inline Column * ColumnsArr::GetColumnByPos(u32 pos)
{
...
inline void ColumnsArr::Update(const std::vector<GameInfo>& game_data)
{
...
inline void ColumnsArr::Show(wxListView * list)
{
...
inline void ColumnsArr::ShowData(wxListView * list)
{
	list->DeleteAllItems();
	list->SetImageList(m_img_list, wxIMAGE_LIST_SMALL);
...
inline void ColumnsArr::LoadSave(bool isLoad, const std::string & path, wxListView * list)
{
	if (isLoad)
		Init();
	else if (list)
	...
	for (auto& column : m_columns)
	{
		if (isLoad)
		{
			std::pair<u32, u32> pair(column.def_pos, column.def_width);
			cfg::try_to_int_pair<u32, '~'>(&pair, cfg::to_string("gui/game_viewer/" + column.name));
			std::tie(column.pos, column.width) = pair;

			column.shown = true;
		}
		else //if (column.shown)
		{
			ASSERT(cfg::from_string("gui/game_viewer/" + column.name, fmt::format("%u~%u", column.pos, column.width)));
