...
static std::unique_ptr<wchar_t[]> to_wchar(const std::string& source)
{
	...
	{
		throw EXCEPTION("MultiByteToWideChar() failed: error %u.", GetLastError());
	}
...
static void to_utf8(std::string& result, const wchar_t* source)
{
	...
	{
		throw EXCEPTION("WideCharToMultiByte() failed: error %u.", GetLastError());
	}
}
...
bool fs::stat(const std::string& path, stat_t& info)
{
	...
	if (!GetFileAttributesExW(to_wchar(path).get(), GetFileExInfoStandard, &attrs))
	{
		...
		default: throw EXCEPTION("Unknown Win32 error: %u (%s).", error, path);
		}
...
bool fs::rename(const std::string& from, const std::string& to)
{
	...
	if (!MoveFileW(to_wchar(from).get(), to_wchar(to).get()))
	{
		...
		{
		case ERROR_PATH_NOT_FOUND: errno = ENOENT; break;
		default: throw EXCEPTION("Unknown Win32 error: %u.\nFrom: %s\nTo: %s", error, from, to);
		}
...
bool fs::truncate_file(const std::string& path, u64 length)
{
	...
	if (!SetFilePointerEx(handle, distance, NULL, FILE_BEGIN) || !SetEndOfFile(handle))
	{
		...
		default: throw EXCEPTION("Unknown Win32 error: %u (length=0x%llx).", error, length);
		}
	...
	public:
		...
		stat_t stat() override
		{
			...
			if (!GetFileInformationByHandleEx(m_handle, FileBasicInfo, &basic_info, sizeof(FILE_BASIC_INFO)))
			{
				...
				{
				case 0:
				default: throw EXCEPTION("Win32 error: %u.", error);
				}
			}
		...
		bool trunc(u64 length) override
		{
			...
			if (!SetFilePointerEx(m_handle, pos, &old, FILE_CURRENT)) // get old position
			{
				...
				{
				case 0:
				default: throw EXCEPTION("Unknown Win32 error: %u.", error);
				}
			...
			if (!SetFilePointerEx(m_handle, old, NULL, FILE_BEGIN) && result) // restore position
			{
				...
				{
					throw EXCEPTION("Win32 error: %u.", error);
				}
			}
		...
		u64 seek(s64 offset, seek_mode whence) override
		{
			...
			const DWORD mode =
				...
				throw EXCEPTION("Invalid whence (0x%x)", whence);
	...
	public:
		...
		stat_t stat() override
		{
			...
			if (::fstat(m_fd, &file_info) != 0)
			{
				...
				{
				case 0:
				default: throw EXCEPTION("Unknown error: %d.", error);
				}
			}
...
bool fs::dir::open(const std::string& path)
{
	...
	auto&& close = gsl::finally(WRAP_EXPR(FindClose(handle)));
		...
		windows_dir(HANDLE handle, WIN32_FIND_DATAW& found)
		{
			while (true)
			{
				...
				if (!FindNextFileW(handle, &found))
				{
					switch (DWORD error = GetLastError())
					{
					case ERROR_NO_MORE_FILES: return;
					default: throw EXCEPTION("Unknown Win32 error: %u.", error);
					}
				}
			}
